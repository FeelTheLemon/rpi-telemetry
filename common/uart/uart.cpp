...
void uart_init(const char * device) { //"/dev/ttyAMA0"
...
int uart_read(char ret[], unsigned int len) {
	...
	if (uart0_filestream != -1) {
		...
		int rx_length = read(uart0_filestream, (void*)ret, len);		//Filestream, buffer to store in, number of bytes to read (max)
		if (rx_length < 0) {
...
int uart_write(char data[], unsigned int len) {
	if (uart0_filestream != -1) {
		int count = write(uart0_filestream, data, sizeof(data) - 1);		//Filestream, bytes to write, number of bytes to write
		if (count < 0) {
